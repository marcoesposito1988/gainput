...
#ifndef GAINPUTINPUTDEVICEKEYBOARDLINUXXI2_H_
#define GAINPUTINPUTDEVICEKEYBOARDLINUXXI2_H_
...
class InputDeviceKeyboardImplLinuxXI2 : public InputDeviceKeyboardImpl
{
public:
    InputDeviceKeyboardImplLinuxXI2(InputManager& manager, InputDevice& device, InputState& state, InputState& previousState) :
		manager_(manager),
		device_(device),
	...
	void HandleRawEvent(XIRawEvent* event)
    {
        ...
        {
            Window win = DefaultRootWindow(event->display);

            XKeyEvent keyEvent;
            keyEvent.type = pressed ? KeyPress : KeyRelease;
            keyEvent.serial = event->serial;
            keyEvent.send_event = event->send_event;
            keyEvent.display = event->display;
            keyEvent.window = win;
            keyEvent.root = win;
            keyEvent.subwindow = win;
            keyEvent.time = event->time;
            keyEvent.x;
            keyEvent.y;
            keyEvent.x_root;
            keyEvent.y_root;
            keyEvent.state = event->flags;
